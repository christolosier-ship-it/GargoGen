const GROUPS = [
  { title: 'Terrain', tools: [['wall','Mur'],['floor','Sol'],['door','Porte'],['entrance','Entrée'],['exit','Sortie']] },
...
export function renderTools(container, selected, onPick) {
  ...
  for (const group of GROUPS) {
    ...
    for (const [value, label] of group.tools) {
      const b = document.createElement('button'); b.textContent = label; b.className = value === selected ? 'active' : ''; b.onclick = () => onPick(value); wrap.appendChild(b);
    }
